...
TEST_F(LCFFlushTest, SplitWhileCompact) {
  Options options;
  ...
  TEST_SYNC_POINT("LCFFlushTest::TEST1");
  fprintf(stdout, "LCFFlushTest::TEST1\n");
  for (int i = 0; i < 51; i++) {
    std::string k = RandomString(&rnd, 8);
    std::string v = RandomString(&rnd, 200);
    db->Put(WriteOptions(), cfh, k, v);
  ...
  TEST_SYNC_POINT("LCFFlushTest::TEST2");
  fprintf(stdout, "LCFFlushTest::TEST2\n");
  dbfull(db)->TEST_WaitForCompact();
  ...
  rocksdb::SyncPoint::GetInstance()->LoadDependency(
    {
      {"CompactionJob::Run():Start" ,"LCFFlushTest::TEST3"},
      { "LCFFlushTest::TEST4", "CompactionJob::Run():End" }
    }
  );
  ...
  // trigger Split
  TEST_SYNC_POINT("LCFFlushTest::TEST3");
  fprintf(stdout, "[LCFFlushTest] First Split Start (1/3)\n");
  dbfull(db)->SplitColumnFamilyFromSstFiles(infos);
  fprintf(stdout, "[LCFFlushTest] First Split Finish (1/3)\n");
  TEST_SYNC_POINT("LCFFlushTest::TEST4");
...
TEST_F(LCFFlushTest, SingleSplit) {
  Options options;
  options.create_if_missing = true;
  options.max_background_jobs =32;
  options.max_write_buffer_number =3;
...
TEST_F(LCFFlushTest, ThreeLevelAfterPut) {
  Options options;
  options.create_if_missing = true;
  options.max_background_jobs =32;
  options.max_write_buffer_number =3;
...
TEST_F(LCFFlushTest, ThreeLevelAfterPut2) {
  Options options;
  options.create_if_missing = true;
  options.allow_column_family_split = true;
  options.atomic_flush = true;
  ...
  f2->largest = InternalKey(Slice(l2), 0, kTypeValue);
  infos.push_back(SplitFileInfo(f2, cfd1));
  fprintf(stdout, "[LCFFlushTest] Second Split Start (2/3)\n");
  dbfull(db)->SplitColumnFamilyFromSstFiles(infos);
  fprintf(stdout, "[LCFFlushTest] Second Split Finish (2/3)\n");
  dbfull(db)->TEST_WaitForSplit();
  sleep(5); 
  ...
  f3->largest = InternalKey(Slice(l3), 0, kTypeValue);
  infos.push_back(SplitFileInfo(f3, cfd));
  fprintf(stdout, "[LCFFlushTest] Third Split Start (3/3)\n");
  dbfull(db)->SplitColumnFamilyFromSstFiles(infos);
  fprintf(stdout, "[LCFFlushTest] Third Split Finish (3/3)\n");
  dbfull(db)->TEST_WaitForSplit();
  sleep(5); 
