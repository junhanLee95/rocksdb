...
class InterCFCompactionIterator {
 public:
  // A wrapper around Compaction. Has a much smaller interface, only what
  // InterCFCompactionIterator uses. Tests can override it.
  class InterCFCompactionProxy {
   public:
    explicit InterCFCompactionProxy(const InterCFCompaction* compaction)
        : compaction_(compaction) {}

    virtual ~InterCFCompactionProxy() = default;
   ...
   protected:
    InterCFCompactionProxy() = default;

   private:
    const InterCFCompaction* compaction_;
  };

  InterCFCompactionIterator(InternalIterator* input, const Comparator* cmp,
                     ...
                     const InterCFCompaction* compaction = nullptr,
                     const CompactionFilter* compaction_filter = nullptr,
                     const std::atomic<bool>* shutting_down = nullptr,
                     const SequenceNumber preserve_deletes_seqnum = 0);

  // Constructor with custom InterCFCompactionProxy, used for tests.
  InterCFCompactionIterator(InternalIterator* input, const Comparator* cmp,
                     MergeHelper* merge_helper, SequenceNumber last_sequence,
                     std::vector<SequenceNumber>* snapshots,
                     SequenceNumber earliest_write_conflict_snapshot,
 ...
 private:
  ...
  std::unique_ptr<InterCFCompactionProxy> compaction_;
