...
Iterator* DBImpl::NewIterator(const ReadOptions& read_options,
  ...
  if (read_options.tailing) {
	...
	if(immutable_db_options_.allow_column_family_split){
		...
		sv = root->GetReferencedSuperVersion(&mutex_);
		InternalIterator* node_iter = new ForwardIterator(this,read_options,root,sv);
		iterators.push_back(node_iter);
		nodes.erase(remove(nodes.begin(), nodes.end(), root->GetPartitionTreeNode()), nodes.end());

		/*
		root=cfd;
		while(root){
		    //fprintf(stdout,"cfd is %d\n",int(root->GetID()));
			sv = root->GetReferencedSuperVersion(&mutex_);
			InternalIterator* node_iter = new ForwardIterator(this,read_options,root,sv);
			iterators.push_back(node_iter);
			nodes.erase(remove(nodes.begin(), nodes.end(), root->GetPartitionTreeNode()), nodes.end());
			//fprintf(stdout,"small key is %s\n",root->GetSmallestKey().c_str());
			auto imm=root->GetPartitionTreeNode()->GetParentNode();
			if(!imm)
				break;
			root=imm->cfd_;
    	...
    	auto iter = new LCFIterator(this, read_options, root, iterators,nodes);
    	result = NewDBIterator(
