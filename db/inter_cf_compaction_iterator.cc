...
#include "db/inter_cf_compaction_iterator.h"
...
InterCFCompactionIterator::InterCFCompactionIterator(
    InternalIterator* input, const Comparator* cmp, MergeHelper* merge_helper,
    SequenceNumber last_sequence, std::vector<SequenceNumber>* snapshots,
    SequenceNumber earliest_write_conflict_snapshot,
    const SnapshotChecker* snapshot_checker, Env* env,
    bool report_detailed_time, bool expect_valid_internal_key,
    CompactionRangeDelAggregator* range_del_agg, const InterCFCompaction* compaction,
    const CompactionFilter* compaction_filter,
    const std::atomic<bool>* shutting_down,
    const SequenceNumber preserve_deletes_seqnum)
    : InterCFCompactionIterator(
          input, cmp, merge_helper, last_sequence, snapshots,
          earliest_write_conflict_snapshot, snapshot_checker, env,
          report_detailed_time, expect_valid_internal_key, range_del_agg,
          std::unique_ptr<InterCFCompactionProxy>(
              compaction ? new InterCFCompactionProxy(compaction) : nullptr),
...
InterCFCompactionIterator::InterCFCompactionIterator(
    ...
    std::unique_ptr<InterCFCompactionProxy> compaction,
    const CompactionFilter* compaction_filter,
    const std::atomic<bool>* shutting_down,
    const SequenceNumber preserve_deletes_seqnum)
    : input_(input),
  ...
  TEST_SYNC_POINT_CALLBACK("InterCFCompactionIterator:AfterInit", compaction_.get());
}
...
InterCFCompactionIterator::~InterCFCompactionIterator() {
  ...
  std::cout << "==================================\n";
  for (auto& it: user_key_flush_cnts_) {
    std::string a = it.first;
    uint64_t b = it.second;
    std::cout << "InterCFCompactionIterator Flush Stats:\n";
    std::cout << a << ": " << b << std::endl;
  }
  for (auto& it: user_key_compaction_cnts_) {
    std::string a = it.first;
    uint64_t b = it.second;
    std::cout << "InterCFCompactionIterator Compaction Stats:\n";
    std::cout << a << ": " << b << std::endl;
  }

  std::cout << "num_uniq_keys : " << num_uniq_keys_ << std::endl;
  std::cout << "total_flush_cnt : " << total_flush_cnt_ << std::endl;
  std::cout << "total_compaction_cnt : " << total_compaction_cnt_ << std::endl;
  std::cout << "==================================\n";*/
  // input_ Iteartor lifetime is longer than pinned_iters_mgr_ lifetime
  input_->SetPinnedItersMgr(nullptr);
}

void InterCFCompactionIterator::ResetRecordCounts() {
...
void InterCFCompactionIterator::SeekToFirst() {
  //std::cout << "c_iter::SeekToFirst()\n";
  NextFromInput();
  PrepareOutput();
}

void InterCFCompactionIterator::Next() {
...
void InterCFCompactionIterator::InvokeFilterIfNeeded(bool* need_skip,
                                              Slice* skip_until) {
...
void InterCFCompactionIterator::NextFromInput() {
  ...
  while (!valid_ && input_->Valid() && !IsShuttingDown()) {
    ...
    TEST_SYNC_POINT_CALLBACK("InterCFCompactionIterator:ProcessKV", &ikey_);
    ...
    if (!has_current_user_key_ ||
      ...
      //std::cout << "Inter-NextFromInput() new \t" << current_user_key_.ToString() << "f_cnt : " << flush_cnt <<std::endl;
      //std::cout << "Inter-NextFromInput() new \t" << current_user_key_.ToString() << "c_cnt : " << compaction_cnt <<std::endl;
      ...
      //std::cout << "Inter-NextFromInput() same\t" << current_user_key_.ToString() << "f_cnt : " << flush_cnt <<std::endl;
      //std::cout << "Inter-NextFromInput() same\t" << current_user_key_.ToString() << "c_cnt : " << compaction_cnt <<std::endl;
      //user_key_flush_cnts_[current_user_key_.ToString()] += flush_cnt;
      //user_key_compaction_cnts_[current_user_key_.ToString()] += compaction_cnt;
...
void InterCFCompactionIterator::PrepareOutput() {
...
inline SequenceNumber InterCFCompactionIterator::findEarliestVisibleSnapshot(
...
inline bool InterCFCompactionIterator::ikeyNotNeededForIncrementalSnapshot() {
  return (!compaction_->preserve_deletes()) ||
         (ikey_.sequence < preserve_deletes_seqnum_);
}

bool InterCFCompactionIterator::IsInEarliestSnapshot(SequenceNumber sequence) {
  assert(snapshot_checker_ != nullptr);
