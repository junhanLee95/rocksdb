...
void InterCFCompaction::GetBoundaryKeys(
    ...
    const std::vector<InterCFCompactionInputFiles>& inputs, Slice* smallest_user_key,
...
std::vector<InterCFCompactionInputFiles> InterCFCompaction::PopulateWithAtomicBoundaries(
    VersionStorageInfo* vstorage, std::vector<InterCFCompactionInputFiles> inputs) {
  const Comparator* ucmp = vstorage->InternalComparator()->user_comparator();
  for (size_t i = 0; i < inputs.size(); i++) {
...
// bottommost level
bool InterCFCompaction::IsBottommostLevel(
    int output_level, VersionStorageInfo* vstorage,
    const std::vector<InterCFCompactionInputFiles>& inputs) {
  int output_l0_idx;
  if (output_level == 0) {
    output_l0_idx = 0;
...
bool InterCFCompaction::TEST_IsBottommostLevel(
    int output_level, VersionStorageInfo* vstorage,
    const std::vector<InterCFCompactionInputFiles>& inputs) {
  return IsBottommostLevel(output_level, vstorage, inputs);
}

bool InterCFCompaction::IsFullCompaction(
    VersionStorageInfo* vstorage,
    const std::vector<InterCFCompactionInputFiles>& inputs) {
...
InterCFCompaction::InterCFCompaction(VersionStorageInfo* vstorage,
                       const ImmutableCFOptions& _immutable_cf_options,
                       const MutableCFOptions& _mutable_cf_options,
                       std::vector<InterCFCompactionInputFiles> _inputs,
...
InterCFCompaction::~InterCFCompaction() {
...
bool InterCFCompaction::InputCompressionMatchesOutput() const {
  ...
  if (matches) {
    TEST_SYNC_POINT("InterCFCompaction::InputCompressionMatchesOutput:Matches");
    return true;
  }
  TEST_SYNC_POINT("InterCFCompaction::InputCompressionMatchesOutput:DidntMatch");
  return matches;
}

bool InterCFCompaction::IsTrivialMove() const {
...
bool InterCFCompaction::KeyNotExistsBeyondOutputLevel(
...
void InterCFCompaction::MarkFilesBeingCompacted(bool mark_as_compacted) {
...
const char* InterCFCompaction::InputLevelSummary(
    InterCFInputLevelSummaryBuffer* scratch) const {
  int len = 0;
  bool is_first = true;
...
uint64_t InterCFCompaction::CalculateTotalInputSize() const {
  uint64_t size = 0;
  for (auto& input_level : inputs_) {
...
void InterCFCompaction::ReleaseCompactionFiles(Status status) {
  MarkFilesBeingCompacted(false);
  (void)status;
  //cfd_->compaction_picker()->ReleaseCompactionFiles(this, status);
}

/*void InterCFCompaction::ResetNextCompactionIndex() {
  assert(input_version_ != nullptr);
...
void InterCFCompaction::Summary(char* output, int len) {
  int write =
...
uint64_t InterCFCompaction::OutputFilePreallocationSize() const {
...
std::unique_ptr<CompactionFilter> InterCFCompaction::CreateCompactionFilter() const {
...
bool InterCFCompaction::IsOutputLevelEmpty() const {
  return inputs_.back().level != output_level_ || inputs_.back().empty();
}

bool InterCFCompaction::ShouldFormSubcompactions() const {
...
uint64_t InterCFCompaction::MaxInputFileCreationTime() const {
...
int InterCFCompaction::GetInputBaseLevel() const {
