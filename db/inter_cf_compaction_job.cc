...
struct InterCFCompactionJob::SubInterCFcompactionState {
  const InterCFCompaction* compaction;
  std::unique_ptr<InterCFCompactionIterator> c_iter;
  ...
  SubInterCFcompactionState(SubInterCFcompactionState&& o) { *this = std::move(o); }

  SubInterCFcompactionState& operator=(SubInterCFcompactionState&& o) {
  ...
  SubInterCFcompactionState(const SubInterCFcompactionState&) = delete;

  SubInterCFcompactionState& operator=(const SubInterCFcompactionState&) = delete;
...
struct InterCFCompactionJob::InterCFCompactionState {
  InterCFCompaction* const inter_cf_compaction;

  // REQUIRED: subcompaction states are stored in order of increasing
  // key-range
  std::vector<InterCFCompactionJob::SubInterCFcompactionState> sub_compact_states;
  Status status;
  ...
  explicit InterCFCompactionState(InterCFCompaction* c)
      : inter_cf_compaction(c),
        total_bytes(0),
        num_input_records(0),
        num_output_records(0) {}
...
void InterCFCompactionJob::AggregateStatistics() {
  for (SubInterCFcompactionState& sc : compact_->sub_compact_states) {
...
/*
InterCFCompactionJob::CompactionJob(
    ...
    : job_id_(job_id),
      compact_(new InterCFCompactionState(compaction)),
...
InterCFCompactionJob::InterCFCompactionJob(
    int job_id, InterCFCompaction* inter_cf_compaction, const ImmutableDBOptions& db_options,
    const EnvOptions env_options, VersionSet* versions,
    const std::atomic<bool>* shutting_down,
    const SequenceNumber preserve_deletes_seqnum, LogBuffer* log_buffer,
    Directory* db_directory, Directory* output_directory, Statistics* stats,
    ...
    : job_id_(job_id),
      ...
      compaction_job_stats_(compaction_job_stats),
      compaction_stats_(inter_cf_compaction->compaction_reason(), 1),
      dbname_(dbname),
      db_options_(db_options),
      env_options_(env_options),
      env_(db_options.env),
      ...
      num_key_range_to_split_(0)
{
  ...
  ThreadStatusUtil::SetColumnFamily(cfd, cfd->ioptions()->env,
                                    db_options_.enable_thread_tracking);
  ThreadStatusUtil::SetThreadOperation(ThreadStatus::OP_COMPACTION);
  ReportStartedCompaction(inter_cf_compaction);
}
...
void InterCFCompactionJob::ReportStartedCompaction(InterCFCompaction* compaction) {
  const auto* cfd = compact_->inter_cf_compaction->column_family_data();
  ThreadStatusUtil::SetColumnFamily(cfd, cfd->ioptions()->env,
                                    db_options_.enable_thread_tracking);
  ...
  ThreadStatusUtil::SetThreadOperationProperty(
      ...
      (static_cast<uint64_t>(compact_->inter_cf_compaction->start_level()) << 32) +
          compact_->inter_cf_compaction->output_level());
...
void InterCFCompactionJob::Prepare() {
  AutoThreadOperationStageUpdater stage_updater(
      ThreadStatus::STAGE_COMPACTION_PREPARE);

  // Generate file_levels_ for compaction berfore making Iterator
  auto* c = compact_->inter_cf_compaction;
  assert(c->column_family_data() != nullptr);
  assert(c->column_family_data()->current()->storage_info()->NumLevelFiles(
             compact_->inter_cf_compaction->level()) > 0);
...
void InterCFCompactionJob::GenSubcompactionBoundaries() {
  auto* c = compact_->inter_cf_compaction;
  ...
  // Get input version from InterCFCompactionState since it's already referenced
  // earlier in SetInputVersioInterCFCompaction::SetInputVersion and will not change
  // when db_mutex_ is released below
  auto* v = compact_->inter_cf_compaction->input_version();
...
Status InterCFCompactionJob::Run() {
  AutoThreadOperationStageUpdater stage_updater(
      ThreadStatus::STAGE_COMPACTION_RUN);
  TEST_SYNC_POINT("InterCFCompactionJob::Run():Start");
  ...
  for (size_t i = 1; i < compact_->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&InterCFCompactionJob::ProcessKeyValueCompaction, this,
  ...
  TEST_SYNC_POINT("InterCFCompactionJob::Run:BeforeVerify");
  ...
  if (status.ok()) {
    ...
    ColumnFamilyData* cfd = compact_->inter_cf_compaction->column_family_data();
    auto prefix_extractor =
        compact_->inter_cf_compaction->mutable_cf_options()->prefix_extractor.get();
  ...
  compact_->inter_cf_compaction->SetOutputTableProperties(std::move(tp));
...
Status InterCFCompactionJob::Install(void) {
  ...
  ROCKS_LOG_BUFFER(
      ...
      bytes_written_per_sec, compact_->inter_cf_compaction->output_level(),
      ...
      CompressionTypeToString(compact_->inter_cf_compaction->output_compression())
          .c_str());
  ...
  stream << "job" << job_id_ << "event"
         << "inter_cf_compaction_finished"
         << "compaction_time_micros" << compaction_stats_.micros
         << "compaction_time_cpu_micros" << compaction_stats_.cpu_micros
         << "output_level" << compact_->inter_cf_compaction->output_level()
...
void InterCFCompactionJob::ProcessKeyValueCompaction(SubInterCFcompactionState* sub_compact) {
  ...
  MergeHelper merge(
      ...
      snapshot_checker_, compact_->inter_cf_compaction->level(),
      db_options_.statistics.get(), shutting_down_);

  TEST_SYNC_POINT("InterCFCompactionJob::Run():Inprogress");
  ...
  Status status;
  sub_compact->c_iter.reset(new InterCFCompactionIterator(
...
void InterCFCompactionJob::RecordDroppedKeys(
...
Status InterCFCompactionJob::FinishCompactionOutputFile(
    const Status& input_status, SubInterCFcompactionState* sub_compact,
  ...
  if (sfm && meta != nullptr && meta->fd.GetPathId() == 0) {
    ...
    if (sfm->IsMaxAllowedSpaceReached()) {
      ...
      TEST_SYNC_POINT(
          "InterCFCompactionJob::FinishCompactionOutputFile:"
...
Status InterCFCompactionJob::InstallCompactionResults(void) {
  ...
  if (!versions_->VerifyCompactionFileConsistency(compaction)) {
    InterCFCompaction::InterCFInputLevelSummaryBuffer inputs_summary;
...
Status InterCFCompactionJob::OpenCompactionOutputFile(
    SubInterCFcompactionState* sub_compact) {
  assert(sub_compact != nullptr);
  assert(sub_compact->builder == nullptr);
  ...
  TEST_SYNC_POINT_CALLBACK("InterCFCompactionJob::OpenCompactionOutputFile",
                           &syncpoint_arg);
#endif
  ...
  SubInterCFcompactionState::Output out;
  out.meta.fd =
...
void InterCFCompactionJob::CleanupCompaction() {
  for (SubInterCFcompactionState& sub_compact : compact_->sub_compact_states) {
...
void InterCFCompactionJob::UpdateCompactionStats() {
  InterCFCompaction* compaction = compact_->inter_cf_compaction;
...
void InterCFCompactionJob::UpdateCompactionInputStatsHelper(int* num_files,
                                                     uint64_t* bytes_read,
                                                     int input_level) {
  const InterCFCompaction* compaction = compact_->inter_cf_compaction;
...
void InterCFCompactionJob::UpdateCompactionJobStats(
...
void InterCFCompactionJob::LogCompaction() {
  InterCFCompaction* compaction = compact_->inter_cf_compaction;
  ColumnFamilyData* cfd = compaction->column_family_data();
  ...
  if (db_options_.info_log_level <= InfoLogLevel::INFO_LEVEL) {
    ...
    stream << "job" << job_id_ << "event"
           << "inter_cf_compaction_started"
           << "compaction_reason" << "UniversalSizeTotal";
