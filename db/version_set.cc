...
std::vector<std::string> VersionSet::GetSmallestAndLargestKeys(Options& options, std::string& dscname) {
  //Options& options, std::string& dscname,
  //                              bool verbose, bool hex, bool json
  // Open the specified manifest file.
  std::vector<std::string> keys_in_dscname;
  ...
  Status s;
  {
    ...
    s = options.env->NewSequentialFile(
        dscname, &file, env_->OptimizeForManifestRead(env_options_));
    if (!s.ok()) {
      std::cerr << "[ERR] opening New SequentialFile of " << dscname << std::endl;
      return keys_in_dscname;
    }
    file_reader.reset(new SequentialFileReader(std::move(file), dscname));
  }
  ...
  if (s.ok()) {
    for (auto cfd : *column_family_set_) {
      ...
      builder->SaveTo(v->storage_info());
      v->PrepareApply(*cfd->GetLatestMutableCFOptions(), false);
      printf("%s \n", v->DebugString(true).c_str());

      for (int level = 0; level < v->storage_info()->num_levels_; level++) {
        const std::vector<FileMetaData*>& files = v->storage_info()->files_[level];
        for (size_t i = 0; i < files.size(); i++) {

          std::cout << "smallest : " << files[i]->smallest.user_key().ToString(true) << std::endl;
          std::cout << "largest : " << files[i]->largest.user_key().ToString(true) << std::endl;

          keys_in_dscname.push_back(files[i]->smallest.user_key().ToString(true));
          keys_in_dscname.push_back(files[i]->largest.user_key().ToString(true));
        }
      }
      delete v;
    }
