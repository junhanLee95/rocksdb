...
namespace rocksdb {
struct SplitJobStats {
  SplitJobStats() { Reset(); }
  void Reset();
  // Aggregate the SplitJobStats from another instance with this one
  void Add(const SplitJobStats& stats);

  // the elapsed time of this split in microseconds.
  uint64_t elapsed_micros;

  // the elapsed CPU time of this split in microseconds.
  uint64_t cpu_micros;

  // the number of split input records.
  uint64_t num_input_records;
  // the number of split input files.
  size_t num_input_files;

  // the number of split output records.
  uint64_t num_output_records;
  // the number of split output files.
  size_t num_output_files;

  // true if the split is a manual split
  bool is_manual_split;

  // the size of the split input in bytes.
  uint64_t total_input_bytes;
  // the size of the split output in bytes.
  ...
  // the number of deletion entries before split. Deletion entries
  // can disappear after split because they expired
